...
function getCommissionAgent(){
    var customer_id = $('#customer_id').val();
    $.ajax({
        url: '/get-commision-agent/' + customer_id,
        dataType: 'json',
        success: function(data) {

            let selected_agent = data[0].id;
            console.log('data: ', selected_agent)

            $('#commission_agent').select2({data: data});
            $('#commission_agent').val(selected_agent).trigger("change");
        },
    });
}
...
$("#sales_order_ids").on("select2:select", function (e) {
    ...
    $.ajax({
        ...
        success: function(result) {
            if (result) {
                if($(result).find('#isOnline').val()=='1'){
                    $('#payment_rows_div').hide();
                }else{
                    $('#payment_rows_div').show();
                }
