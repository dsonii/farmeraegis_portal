$(document).ready(function() {
    ...
    $(document).on('click', '.remove_demand_entry_row', function() {
        swal({
            title: LANG.sure,
            icon: 'warning',
            buttons: true,
    ...
    $(document).on('change', '.demand_quantity', function() {
        var row = $(this).closest('tr');
        var quantity = __read_number($(this), true);
        var demand_before_tax = __read_number(row.find('input.demand_unit_cost'), true);
        var demand_after_tax = __read_number(
            row.find('input.demand_unit_cost_after_tax'),
            true
        );

        //Calculate sub totals
        var sub_total_before_tax = quantity * demand_before_tax;
        var sub_total_after_tax = quantity * demand_after_tax;
    ...
    $(document).on('change', '.demand_unit_cost_without_discount', function() {
        var demand_before_discount = __read_number($(this), true);

        var row = $(this).closest('tr');
        var discount_percent = __read_number(row.find('input.inline_discounts'), true);
        var quantity = __read_number(row.find('input.demand_quantity'), true);

        //Calculations.
        var demand_before_tax =
            parseFloat(demand_before_discount) -
            __calculate_amount('percentage', discount_percent, demand_before_discount);

        __write_number(row.find('input.demand_unit_cost'), demand_before_tax, true);
        ...
        var tax_rate = parseFloat(
            row
                .find('select.demand_line_tax_id')
                .find(':selected')
                .data('tax_amount')
        );
        var tax = __calculate_amount('percentage', tax_rate, demand_before_tax);

        var demand_after_tax = demand_before_tax + tax;
        var sub_total_after_tax = quantity * demand_after_tax;
        ...
        __write_number(row.find('input.demand_unit_cost_after_tax'), demand_after_tax, true);
        row.find('.row_subtotal_after_tax').text(
            __currency_trans_from_en(sub_total_after_tax, false, true)
        );
        __write_number(row.find('input.row_subtotal_after_tax_hidden'), sub_total_after_tax, true);

        row.find('.demand_product_unit_tax_text').text(
            __currency_trans_from_en(tax, false, true)
        );
        __write_number(row.find('input.demand_product_unit_tax'), tax, true);
    ...
    $(document).on('change', '.inline_discounts', function() {
        ...
        var quantity = __read_number(row.find('input.demand_quantity'), true);
        var demand_before_discount = __read_number(
            row.find('input.demand_unit_cost_without_discount'),
            true
        );
    ...
    $(document).on('change', '.demand_unit_cost', function() {
        var row = $(this).closest('tr');
        var quantity = __read_number(row.find('input.demand_quantity'), true);
        var demand_before_tax = __read_number($(this), true);

        var sub_total_before_tax = quantity * demand_before_tax;

        //Update unit cost price before discount
        var discount_percent = __read_number(row.find('input.inline_discounts'), true);
        var demand_before_discount = __get_principle(demand_before_tax, discount_percent, true);
        __write_number(
            row.find('input.demand_unit_cost_without_discount'),
            demand_before_discount,
            true
        );
        ...
        //row.find('.demand_product_unit_tax_text').text( tax );
        __write_number(row.find('input.demand_unit_cost_after_tax'), demand_after_tax, true);
        row.find('.row_subtotal_after_tax').text(
            __currency_trans_from_en(sub_total_after_tax, false, true)
        );
    ...
    $(document).on('change', 'select.demand_line_tax_id', function() {
        var row = $(this).closest('tr');
        var demand_before_tax = __read_number(row.find('.demand_unit_cost'), true);
        var quantity = __read_number(row.find('input.demand_quantity'), true);
    ...
    $(document).on('change', '.demand_unit_cost_after_tax', function() {
        var row = $(this).closest('tr');
        var demand_after_tax = __read_number($(this), true);
        var quantity = __read_number(row.find('input.demand_quantity'), true);

        var sub_total_after_tax = demand_after_tax * quantity;
        ...
        var demand_before_tax = __get_principle(demand_after_tax, tax_rate);
        var sub_total_before_tax = quantity * demand_before_tax;
        var tax = __calculate_amount('percentage', tax_rate, demand_before_tax);
        ...
        __write_number(row.find('.demand_unit_cost'), demand_before_tax, true);
        ...
        row.find('.demand_product_unit_tax_text').text(__currency_trans_from_en(tax, true, true));
        __write_number(row.find('input.demand_product_unit_tax'), tax);
    ...
    //Assign Raw Material table
    assign_raw_material = $('#assign_raw_material').DataTable({
        processing: true,
        serverSide: true,
        scrollY: "75vh",
        scrollX:        true,
        scrollCollapse: true,
        ajax: {
            url: '/assign-raw-material',
            data: function(d) {
                if ($('#demand_list_filter_location_id').length) {
                    d.location_id = $('#demand_list_filter_location_id').val();
                }
                if ($('#demand_list_filter_supplier_id').length) {
                    d.supplier_id = $('#demand_list_filter_supplier_id').val();
                }
                if ($('#demand_list_filter_status').length) {
        ...
        fnDrawCallback: function(oSettings) {
            __currency_convert_recursively($('#assign_raw_material'));
        },
    ...
    //Completed Demand table
    completed_demand_table = $('#completed_demand_table').DataTable({
        processing: true,
        serverSide: true,
        scrollY: "75vh",
        scrollX:        true,
        ...
        ajax: {
            ...
            data: function(d) {
                ...
                if ($('#demand_list_filter_payment_status').length) {
                    d.payment_status = $('#demand_list_filter_payment_status').val();
                }
                if ($('#demand_list_filter_status').length) {
                    d.status = $('#demand_list_filter_status').val();
                }
        ...
        fnDrawCallback: function(oSettings) {
            __currency_convert_recursively($('#demand_table'));
        },
        createdRow: function(row, data, dataIndex) {
            $(row)
                .find('td:eq(5)')
    ...
    //Pending Demand table
    pending_demand_table = $('#pending_demand_table').DataTable({
        processing: true,
        serverSide: true,
        scrollY: "75vh",
        scrollX:        true,
    ...
    //Demand table
    demand_table = $('#demand_table').DataTable({
        processing: true,
        serverSide: true,
        scrollY: "75vh",
        scrollX:        true,
    ...
    $(document).on(
        ...
        '#demand_list_filter_location_id, \
                    #demand_list_filter_supplier_id, #demand_list_filter_payment_status,\
                     #demand_list_filter_status',
        function() {
    ...
    $(document).on('change', '.profit_percent', function() {
        ...
        var demand_unit_cost = __read_number(row.find('input.demand_unit_cost_after_tax'), true);
        var default_sell_price =
            parseFloat(demand_unit_cost) +
            __calculate_amount('percentage', profit_percent, demand_unit_cost);
    ...
    $('table#demand_entry_table').on('change', 'select.sub_unit', function() {
        ...
        var cp_element = tr.find('input.demand_unit_cost_without_discount');
...
function get_demand_entry_row(product_id, variation_id) {
    if (product_id) {
        ...
        if ($('#is_demand_order').length) {
            data.is_demand_order = true;
        }
        $.ajax({
            method: 'POST',
            url: '/demands/get_demand_entry_row',
            dataType: 'html',
            data: data,
            success: function(result) {
                append_demand_lines(result, row_count);
            },
        });
    }
}

function append_demand_lines(data, row_count) {
    $(data)
        .find('.demand_quantity')
        .each(function() {
            row = $(this).closest('tr');

            $('#demand_entry_table tbody').append(
                update_demand_entry_row_values(row)
            );
    ...
    if ($(data).find('.demand_quantity').length) {
        $('#row_count').val(
            $(data).find('.demand_quantity').length + parseInt(row_count)
        );
    }
}

function update_demand_entry_row_values(row) {

    console.log('i am here: ', row)

    if (typeof row != 'undefined') {
        var quantity = __read_number(row.find('.demand_quantity'), true);
        var unit_cost_price = __read_number(row.find('.demand_unit_cost'), true);
        var row_subtotal_before_tax = quantity * unit_cost_price;

        var tax_rate = parseFloat(
            $('option:selected', row.find('.demand_line_tax_id')).attr('data-tax_amount')
        );
        ...
        __write_number(row.find('.demand_product_unit_tax'), unit_product_tax, true);
        row.find('.demand_product_unit_tax_text').text(
            __currency_trans_from_en(unit_product_tax, false, true)
        );
        row.find('.demand_unit_cost_after_tax').text(
...
function update_row_price_for_exchange_rate(row) {
    ...
    var demand_unit_cost_without_discount =
        __read_number(row.find('.demand_unit_cost_without_discount'), true) / exchange_rate;
    __write_number(
        row.find('.demand_unit_cost_without_discount'),
        demand_unit_cost_without_discount,
        true
    );

    var demand_unit_cost = __read_number(row.find('.demand_unit_cost'), true) / exchange_rate;
    __write_number(row.find('.demand_unit_cost'), demand_unit_cost, true);
    ...
    var demand_product_unit_tax =
        __read_number(row.find('.demand_product_unit_tax'), true) / exchange_rate;
    __write_number(row.find('input.demand_product_unit_tax'), demand_product_unit_tax, true);
    row.find('.demand_product_unit_tax_text').text(
        __currency_trans_from_en(demand_product_unit_tax, false, true)
    );

    var demand_unit_cost_after_tax =
        __read_number(row.find('.demand_unit_cost_after_tax'), true) / exchange_rate;
    __write_number(
        row.find('input.demand_unit_cost_after_tax'),
        demand_unit_cost_after_tax,
        true
    );
...
function update_inline_profit_percentage(row) {
    ...
    var demand_after_tax = __read_number(row.find('input.demand_unit_cost_after_tax'), true);
    var profit_percent = __get_rate(demand_after_tax, default_sell_price_in_base_currency);
...
function update_table_total() {
    ...
    $('#demand_entry_table tbody')
        .find('tr')
        .each(function() {
            total_quantity += __read_number($(this).find('.demand_quantity'), true);
...
function update_table_sr_number() {
    var sr_number = 1;
    $('table#demand_entry_table tbody')
        .find('.sr_number')
        .each(function() {
            $(this).text(sr_number);
            sr_number++;
...
$(document).on('click', 'button#submit_demand_form', function(e) {
    e.preventDefault();

    //Check if product is present or not.
    if ($('table#demand_entry_table tbody tr').length <= 0) {
        toastr.warning(LANG.no_products_added);
        $('input#search_product').select();
        return false;
    }

    $('form#add_demand_form').validate({
        rules: {
            ref_no: {
                remote: {
    ...
    if ($('form#add_demand_form').valid()) {
        $(this).attr('disabled', true);
        $('form#add_demand_form').submit();
    }
});
...
$(document).on('change', '#location_id', function(){
    if ($('#demand_order_ids').length) {
        location_id = $(this).val();
        $.ajax({
            url: '/get-demand-orders/' + location_id,
            dataType: 'json',
            success: function(data) {

                $('#demand_order_ids').select2('destroy').empty().select2({data: data});
                $('#demand_entry_table tbody').find('tr').each( function(){
                    if (typeof($(this).data('demand_order_id')) !== 'undefined') {
                        $(this).remove();
                    }
                });
            },
        ...
        $.ajax({
            url: '/get-location-details/' + location_id,
            dataType: 'json',
            success: function(data) {
                let selected_location = data[0].id;
                $('#supplier_id').select2({data: data});
                $('#supplier_id').val(selected_location).trigger("change");
            },
        });
    }
});

$(document).on('change', '#location_ids', function(){
    if ($('#demand_order_ids').length) {
        location_id = $(this).val();
        $.ajax({
            url: '/get-demand-orders-qty-finder/' + location_id,
            dataType: 'json',
            success: function(data) {
...
$("#demand_order_ids").on("select2:select", function (e) {
    var demand_order_id = e.params.data.id;
    var row_count = $('#row_count').val();
    $.ajax({
        url: '/get-demand-order-lines/' + demand_order_id + '?row_count=' + row_count,
        dataType: 'html',
        success: function(data) {
            append_demand_lines(data, row_count);
        },
    });

    let demandIds = $(this).val();

    $.ajax({
        url: '/get-demand-order-lines-product/' + demandIds,
        dataType: 'json',
        success: function(data) {
            $('#demand_product_ids').select2('destroy').empty().select2({data: data});
        },
    });

});

$("#demand_order_ids").on("select2:unselect", function (e) {
    var demand_order_id = e.params.data.id;
    $('#demand_entry_table tbody').find('tr').each( function(){
        if (typeof($(this).data('demand_order_id')) !== 'undefined' 
            && $(this).data('demand_order_id') == demand_order_id) {
            $(this).remove();
        }
    });
