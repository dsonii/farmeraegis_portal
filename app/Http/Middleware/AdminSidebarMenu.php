...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            //demands dropdown
            if (in_array('demands', $enabled_modules) && (auth()->user()->can('demand.view') || auth()->user()->can('demand.create') || auth()->user()->can('demand.update'))) {
                $menu->dropdown(
                    __('demand.demands'),
                    function ($sub) use ($common_settings) {
                        if ((auth()->user()->can('demand_order.view_all') || auth()->user()->can('demand_order.view_own')) ) {
                            $sub->url(
                                action('DemandOrderController@index'),
                                __('lang_v1.demand_order'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'demand-order']
                            );
                        }
                        if (auth()->user()->can('demand.view') || auth()->user()->can('view_own_demand')) {
                            $sub->url(
                                action('DemandController@index'),
                                __('demand.list_demand'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'demands' && request()->segment(2) == null]
                            );
                        }

                        if (auth()->user()->can('demand.completed') || auth()->user()->can('view_own_demand')) {
                        
                            $sub->url(
                                action('DemandController@completed'),
                                __('lang_v1.completed_demand'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'completed-demands']
                            );
                        }
                        if (auth()->user()->can('demand.pending') || auth()->user()->can('view_own_demand')) {
                            $sub->url(
                                action('DemandController@pending'),
                                __('lang_v1.pending_demand'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'pending-demands']
                            );
                        }

                        if (auth()->user()->can('demand.create')) {
                            $sub->url(
                                action('DemandController@create'),
                                __('demand.add_demand'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'demands' && request()->segment(2) == 'create']
                            );
                        }
                        if (auth()->user()->can('demand.update')) {
                            $sub->url(
                                action('DemandReturnController@index'),
                                __('lang_v1.list_demand_return'),
                                ['icon' => 'fa fas fa-undo', 'active' => request()->segment(1) == 'demand-return']
                            );
                        }
                        if (auth()->user()->can('demand.update')) {
                            $sub->url(
                                action('DemandController@demandQtyFinder'),
                                __('lang_v1.demand_qty_finder'),
                                ['icon' => 'fa fas fa-undo', 'active' => request()->segment(1) == 'demand-qty-finder']
                            );
                        }
                    },
                    ['icon' => 'fa fas fa-arrow-circle-down', 'id' => 'tour_step10']
                )->order(24);
            }
            ...
            if (in_array('purchases', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create') || auth()->user()->can('purchase.update'))) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($common_settings) {
                        ...
                        if (!empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.completed') || auth()->user()->can('purchase_order.view_own')) ) {
                            $sub->url(
                                action('PurchaseOrderController@completed'),
                                __('lang_v1.complete_purchase_order'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'completed-purchase-order']
                            );
                        }
                        if (!empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.pending') || auth()->user()->can('purchase_order.view_own')) ) {
                            $sub->url(
                                action('PurchaseOrderController@pending'),
                                __('lang_v1.pending_purchase_order'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'pending-purchase-order']
                            );
                        }
                        ...
                        if (auth()->user()->can('purchase_order.quality')) {
                            $sub->url(
                                action('QualityCheckController@index'),
                                "Quality Check",
                                ['icon' => 'fa fas fa-undo', 'active' => request()->segment(1) == 'quality-chek']
                            );
                        }
                    },
            ...
            //Assign Raw Material dropdown
            if (in_array('assign_raw_materials', $enabled_modules) && (auth()->user()->can('assign-raw-material.view') || auth()->user()->can('assign-raw-material.create') || auth()->user()->can('assign-raw-material.update'))) {
                $menu->dropdown(
                    __('assignment.assign_raw_materials'),
                    function ($sub) use ($common_settings) {
                        if (auth()->user()->can('assign_raw_materials.view') || auth()->user()->can('view_own_purchase')) {
                            $sub->url(
                                action('AssignRawMaterialController@index'),
                                __('assignment.list_assign_raw_material'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'assign-raw-material' && request()->segment(2) == null]
                            );
                        }
                        if (auth()->user()->can('assign_raw_materials.create')) {
                            $sub->url(
                                action('AssignRawMaterialController@create'),
                                __('assignment.add_assign_raw_material'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'assign-raw-material' && request()->segment(2) == 'create']
                            );
                        }
                    },
                    ['icon' => 'fa fas fa-tasks', 'id' => 'tour_step9']
                )->order(25);
            }
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
                        if (!empty($pos_settings['enable_sales_order']) && ($is_admin ||auth()->user()->hasAnyPermission(['so.view_own', 'so.view_all', 'so.create'])) ) {
                            $sub->url(
                                ...
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'sales-order' && empty(request()->get('filter-status')) && empty(request()->get('filter'))]
                            );
                        }
                        
                        $sub->url(
                            action('SalesOrderController@index', ['filter-status' => 'ordered']),
                            
                            "Pending Order",
                            ['icon' => 'fa fas fa-list', 'active' => request()->get('filter-status') == 'ordered']
                        );
                        
                        $sub->url(
                            action('SalesOrderController@index', ['filter' => 'ordered']),
                            "Ordered",
                            ['icon' => 'fa fas fa-list', 'active' => request()->filter == 'ordered']
                        );
                        
                        $sub->url(
                            action('SalesOrderController@index', ['filter' => 'packed']),
                            "Packed",
                            ['icon' => 'fa fas fa-list', 'active' => request()->filter == 'packed']
                        );
                        
                        $sub->url(
                            action('SalesOrderController@index', ['filter' => 'shipped']),
                            "Shipped",
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'shipped']
                        );
                        
                        $sub->url(
                            action('SalesOrderController@index', ['filter' => 'delivered']),
                            "Delivered",
                            ['icon' => 'fa fas fa-list', 'active' => request()->filter == 'delivered']
                        );
                        
                        $sub->url(
                            action('SalesOrderController@index', ['filter' => 'cancelled']),
                            "Cancelled",
                            ['icon' => 'fa fas fa-list', 'active' => request()->filter == 'cancelled']
                        );
                        
                        $sub->url(
                            action('SalesOrderController@index', ['filter' => 'returned']),
                            "Returned",
                            ['icon' => 'fa fas fa-list', 'active' => request()->filter == 'returned']
                        );
                        
                         $sub->url(
                            action('SalesOrderController@index', ['filter' => 'failed']),
                            "Failed",
                            ['icon' => 'fa fas fa-list', 'active' => request()->filter == 'failed']
                        );

                        $sub->url(
                            action('SellController@completed'),
                            __('lang_v1.completed_invoice'),
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'completed-sales']
                        );
                        
                        $sub->url(
                            action('SellController@pending'),
                            __('lang_v1.pending_invoice'),
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'pending-sales']
                        );
            ...
            //Wallet dropdown
            if ((auth()->user()->can('wallets.all'))) {
                $menu->dropdown(
                    __('wallet.wallets'),
                    function ($sub) {
                        $sub->url(
                            action('WalletController@index'),
                            __('wallet.list_wallets'),
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'wallets' && request()->segment(2) == null]
                        );

                        if (auth()->user()->can('expense.add')) {
                            $sub->url(
                                action('WalletController@create'),
                                __('wallet.add_wallets'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'wallets' && request()->segment(2) == 'create']
                            );
                        }
                    },
                    ['icon' => 'fa fas fa-minus-circle']
