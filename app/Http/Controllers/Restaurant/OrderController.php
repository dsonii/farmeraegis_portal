...
class OrderController extends Controller
{
    ...
    public function markAsServed(Request $request, $id)
    {
        ...
        try {
            ...
            $transaction->shipping_status = 'packed';
            $transaction->save();
            
            if($request->transaction_sell_line_id){
                foreach($request->transaction_sell_line_id as $key => $val){
                    $sellLie = TransactionSellLine::find($val);
                    $sellLie->pack_qty = $request->pack_qty[$key];
                    $sellLie->save();
                }                
            }
