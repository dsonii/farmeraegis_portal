...
class TransactionUtil extends Util
{
     ...
     * @return boolean
     */
    public function createOrUpdatePaymentLines($transaction, $payments, $business_id = null, $user_id = null, $uf_data = true, $sellOrderDetails='')
    {
        $payments_formatted = [];
        $edit_ids = [0];
        ...
        if(!empty($sellOrderDetails) and $sellOrderDetails->is_online){
            $payments = $sellOrderDetails->payment_lines->toArray();
        }
    ...
    */
     ...
     * @return string
     */
    public function updatePaymentStatus($transaction_id, $final_amount = null, $sellOrderDetails = '')
    {
        $status = $this->calculatePaymentStatus($transaction_id, $final_amount);
       
        if(!empty($sellOrderDetails) && !empty($sellOrderDetails->is_online)){
            
            $status = $sellOrderDetails->payment_status;
    ...
    /**
     * Demand currency details
     *
     * @param int $business_id
     *
     * @return object
     */
    public function demandCurrencyDetails($business_id)
    {
        $business = Business::find($business_id);
        $output = ['demand_in_diff_currency' => false,
                    'p_exchange_rate' => 1,
                    'decimal_seperator' => '.',
                    'thousand_seperator' => ',',
                    'symbol' => '',
        ...
        //Check if diff currency is used or not.
        if ($business->purchase_in_diff_currency == 1) {
            $output['demand_in_diff_currency'] = true;
            $output['p_exchange_rate'] = $business->p_exchange_rate;
    ...
    * @return object
    */
    public function getListDemands($business_id)
    {
        $demands = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                    ->join(
                        'business_locations AS BS',
                        'transactions.location_id',
                        '=',
                    ...
                    ->leftJoin('users as u', 'transactions.created_by', '=', 'u.id')
                    ->where('transactions.business_id', $business_id)
                    ->where('transactions.type', 'demand')
                    ->select(
                        'transactions.id',
                        'transactions.document',
                        'transactions.transaction_date',
    ...
    */
    ...
    public function updateDemandOrderStatus($demand_order_ids = [])
    {
        foreach ($demand_order_ids as $demand_order_id) {
            $demand_order = Transaction::with(['demand_lines'])->find($demand_order_id);

            if (empty($demand_order)) {
                continue;
            }
            $total_ordered = $demand_order->demand_lines->sum('quantity');
            $total_received = $demand_order->demand_lines->sum('po_quantity_demandd');
    ...
    public function createWallet($request, $business_id, $user_id, $format_data = true)
    {
        $transaction_data = $request->only([ 'ref_no', 'transaction_date', 
            'location_id', 'final_total', 'additional_notes', 
            'expense_category_id', 'contact_id']);
    ...
    public function createWalletFromApi($request, $business_id, $user_id, $format_data = true)
    {
        // $transaction_data = $request->only([ 'ref_no', 'transaction_date', 
        //     'location_id', 'final_total', 'additional_notes', 
        //     'expense_category_id', 'contact_id']);
        ...
        $transaction_data['business_id'] = $business_id;
        $transaction_data['location_id'] = 6;
        $transaction_data['contact_id'] = $request['contact_id'];
        $transaction_data['created_by'] = $user_id;
        $transaction_data['type'] = 'wallet';
        $transaction_data['status'] = 'final';
        $transaction_data['additional_notes'] = $request['additional_notes'];
        $transaction_data['payment_status'] = 'paid';
        $transaction_data['final_total'] = $format_data ? $this->num_uf($request['final_total']) : $request['final_total'];
        ...
        $payments = !empty($request['payment']) ? $request['payment'] : [];
        //add expense payment
        $this->createOrUpdatePaymentLines($transaction, $payments, $business_id);
