...
class ProductUtil extends Util
{
    ...
    public function getDetailsFromVariation($variation_id, $business_id, $location_id = null, $check_qty = true)
    {
        ...
        if(empty($query->count())){
            dd("business-".$business_id,"varition-".$variation_id,"location-".$location_id,"product-".$query->get());
        }
    ...
    /**
     * Updates variation from demand screen
     *
     * @param array $variation_data
     *
     * @return void
     */
    public function updateProductFromDemand($variation_data)
    {
        ...
        if (($variation_details->default_demand_price != $variation_data['pp_without_discount']) ||
            ($variation_details->sell_price_inc_tax != $variation_data['sell_price_inc_tax'])
            ) {
            //Set default demand price exc. tax
            $variation_details->default_demand_price = $variation_data['pp_without_discount'];

            //Set default demand price inc. tax
            $variation_details->dpp_inc_tax = $this->calc_percentage($variation_details->default_demand_price, $tax_rate, $variation_details->default_demand_price);
            ...
            //set profit margin
            $variation_details->profit_percent = $this->get_percent($variation_details->default_demand_price, $variation_details->default_sell_price);
...
/**
     * Add/Edit transaction demand lines
     *
     * @param object $transaction
     ...
     * @return array
     */
    public function createOrUpdateDemandLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        $updated_demand_lines = [];
        $updated_demand_line_ids = [0];
        ...
        foreach ($input_data as $data) {
            ...
            $old_qty = 0;
            //update existing demand line
            if (isset($data['demand_line_id'])) {
                $demand_line = DemandLine::findOrFail($data['demand_line_id']);
                $updated_demand_line_ids[] = $demand_line->id;
                $old_qty = $demand_line->quantity;

                $this->updateProductStock($before_status, $transaction, $data['product_id'], $data['variation_id'], $new_quantity, $demand_line->quantity, $currency_details);
            } else {
                //create newly added demand lines
                $demand_line = new DemandLine();
                $demand_line->product_id = $data['product_id'];
                $demand_line->variation_id = $data['variation_id'];
            ...
            $demand_line->quantity = $new_quantity;
            $demand_line->pp_without_discount = ($this->num_uf($data['pp_without_discount'], $currency_details)*$exchange_rate) / $multiplier;
            $demand_line->discount_percent = $this->num_uf($data['discount_percent'], $currency_details);
            $demand_line->demand_price = ($this->num_uf($data['demand_price'], $currency_details)*$exchange_rate) / $multiplier;
            $demand_line->demand_price_inc_tax = ($this->num_uf($data['demand_price_inc_tax'], $currency_details)*$exchange_rate) / $multiplier;
            $demand_line->item_tax = ($this->num_uf($data['item_tax'], $currency_details)*$exchange_rate) / $multiplier;
            $demand_line->tax_id = $data['demand_line_tax_id'];
            $demand_line->lot_number = !empty($data['lot_number']) ? $data['lot_number'] : null;
            $demand_line->mfg_date = !empty($data['mfg_date']) ? $this->uf_date($data['mfg_date']) : null;
            $demand_line->exp_date = !empty($data['exp_date']) ? $this->uf_date($data['exp_date']) : null;
            $demand_line->sub_unit_id = !empty($data['sub_unit_id']) ? $data['sub_unit_id'] : null;
            $demand_line->demand_order_line_id = !empty($data['demand_order_line_id']) ? $data['demand_order_line_id'] : null;
        
            $updated_demand_lines[] = $demand_line;
            ...
            if ($enable_product_editing == 1) {
                ...
                $variation_data['variation_id'] = $demand_line->variation_id;
                $variation_data['demand_price'] = $demand_line->demand_price;
             
                $this->updateProductFromDemand($variation_data);
            }

            //Update demand order line quantity received
            $this->updateDemandOrderLine($demand_line->demand_order_line_id, $demand_line->quantity, $old_qty);
        }

        //unset deleted demand lines
        $delete_demand_line_ids = [];
        $delete_demand_lines = null;
        if (!empty($updated_demand_line_ids)) {
            $delete_demand_lines = DemandLine::where('transaction_id', $transaction->id)
                    ->whereNotIn('id', $updated_demand_line_ids)
                    ->get();

            if ($delete_demand_lines->count()) {
                foreach ($delete_demand_lines as $delete_demand_line) {
                    $delete_demand_line_ids[] = $delete_demand_line->id;

                    //decrease deleted only if previous status was received
                    if ($before_status == 'received') {
                        $this->decreaseProductQuantity(
                            $delete_demand_line->product_id,
                            $delete_demand_line->variation_id,
                            $transaction->location_id,
                            $delete_demand_line->quantity
                        );
                    }

                    //If demand order line set decrease quntity
                    if (!empty($delete_demand_line->demand_order_line_id)) {
                        $this->updateDemandOrderLine($delete_demand_line->demand_order_line_id, 0, $delete_demand_line->quantity);
                    }
                }

                //unset if demand order line from demand lines if exists
                if ($transaction->type == 'demand_order') {
                    DemandLine::whereIn('demand_order_line_id', $delete_demand_line_ids)
                        ->update(['demand_order_line_id' => null]);
                }

                //Delete deleted demand lines
        ...
        //update demand lines
        if (!empty($updated_demand_lines)) {
            $transaction->demand_lines()->saveMany($updated_demand_lines);
        }

        return $delete_demand_lines;
    }
     ...
     * @return array
     */
    public function createOrUpdateDamandLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        $updated_demand_lines = [];
        $updated_demand_line_ids = [0];
        $exchange_rate = !empty($transaction->exchange_rate) ? $transaction->exchange_rate : 1;
    ...
    public function updateDemandOrderLine($demand_order_line_id, $new_qty, $old_qty = 0)
    {
        $diff = $new_qty - $old_qty;
        if (!empty($demand_order_line_id) && !empty($diff)) {
            $demand_order_line = DemandLine::find($demand_order_line_id);
            $demand_order_line->po_quantity_demanded += ($diff);
            $demand_order_line->save();
        }
    }
    ...
    /**
     * Recalculates demand line data according to subunit data
     *
     * @param integer $demand_line
     * @param integer $business_id
     *
     * @return array
     */
    public function changeDemandLineUnit($demand_line, $business_id)
    {
        $base_unit = $demand_line->product->unit;
        $sub_units = $base_unit->sub_units;

        $sub_unit_id = $demand_line->sub_unit_id;
        ...
        if (!empty($sub_unit)) {
            ...
            $demand_line->quantity = $demand_line->quantity / $multiplier;
            $demand_line->pp_without_discount = $demand_line->pp_without_discount * $multiplier;
            $demand_line->demand_price = $demand_line->demand_price * $multiplier;
            $demand_line->demand_price_inc_tax = $demand_line->demand_price_inc_tax * $multiplier;
            $demand_line->item_tax = $demand_line->item_tax * $multiplier;
            $demand_line->quantity_returned = $demand_line->quantity_returned / $multiplier;
            $demand_line->quantity_sold = $demand_line->quantity_sold / $multiplier;
            $demand_line->quantity_adjusted = $demand_line->quantity_adjusted / $multiplier;
        }

        //SubUnits
        $demand_line->sub_units_options = $this->getSubUnits($business_id, $base_unit->id, false, $demand_line->product_id);
