...
class Contact extends Authenticatable
{
    ...
    */
    ...
    {
        return $this->hasMany(\App\Address::class,"user_id","id")->where("address_type","contact");
    }
    ...
    public static function contactDropdown($business_id, $exclude_default = false, $prepend_none = true, $append_id = true)
    {
        ...
        if ($append_id) {
            $query->select(
                DB::raw("IF(contact_id IS NULL OR contact_id='', name, CONCAT(name, ' - ', COALESCE(supplier_business_name, ''), '(', contact_id, ')', ' - ', mobile)) AS supplier"),
                'id'
                    );
        } else {
            $query->select(
