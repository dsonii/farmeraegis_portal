<div class="modal-dialog modal-xl" role="document">
  <div class="modal-content">
    <div class="modal-header">
      ...
      <h4 class="modal-title" id="modalTitle"> @lang('lang_v1.demand_return_details') (<b>@lang('demand.ref_no'):</b> #{{ $demand->return_parent->ref_no ?? $demand->ref_no}})
        </h4>
    </div>

    <div class="modal-body">
      <div class="row">
        @if(!empty($demand->return_parent))
        <div class="col-sm-6 col-xs-6">
            <h4>@lang('lang_v1.demand_return_details'):</h4>
            <strong>@lang('lang_v1.return_date'):</strong> {{@format_date($demand->return_parent->transaction_date)}}<br>
        ...
        <div class="col-sm-6 col-xs-6">
            <h4>@lang('demand.demand_details'):</h4>
        ...
        @else
            <div class="col-sm-6 col-xs-6">
                <h4>@lang('lang_v1.demand_return_details'):</h4>
                <strong>@lang('lang_v1.return_date'):</strong> {{@format_date($demand->transaction_date)}}<br>
                <strong>@lang('demand.supplier'):</strong> {{ $demand->contact->name ?? '' }} <br>
                <strong>@lang('demand.business_location'):</strong> {{ $demand->location->name }}
            </div>
        @endif
            ...
            @if($demand->document_path)
                <div class="col-md-12">
                    <a href="{{$demand->document_path}}" 
                      download="{{$demand->document_name}}" class="btn btn-sm btn-success pull-right no-print">
      ...
      <div class="row">
        <div class="col-sm-12">
          ...
          <tbody>
              ...
              @if($demand_line->quantity_returned == 0)
                @continue
              @endif

              @php
                $unit_name = $demand_line->product->unit->short_name;
                if(!empty($demand_line->sub_unit)) {
              ...
              <tr>
                  ...
                  <td>{{@format_quantity($demand_line->quantity_returned)}} {{$unit_name}}</td>
                  <td>
                    @php
                      $line_total = $demand_line->demand_price_inc_tax * $demand_line->quantity_returned;
      ...
      <div class="col-sm-6 col-sm-offset-6 col-xs-6 col-xs-offset-6">
        <table class="table">
          ...
          <tr>
            ...
            <td></td>
            <td><span class="display_currency pull-right" data-currency_symbol="true" >{{ $demand->return_parent->final_total ??  $demand->final_total }}</span></td>
          </tr>
        </table>
      </div>
    </div>
