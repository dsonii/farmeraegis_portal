<tr class="product_row">
    ...
    <td>
        ...
        @if(!empty($edit))
            <input type="hidden" value="{{$product->demand_line_id}}" 
            name="products[{{$row_index}}][demand_line_id]">
            @php
                $qty = $product->quantity_returned;
                $demand_price = $product->demand_price;
            @endphp
        @else
            @php
                $qty = 1;
                $demand_price = $product->last_demanded_price;
            @endphp
        @endif
    ...
    <td>
        <input type="text" name="products[{{$row_index}}][unit_price]" class="form-control product_unit_price input_number" value="{{@num_format($demand_price)}}">
    </td>
    <td>
        <input type="text" readonly name="products[{{$row_index}}][price]" class="form-control product_line_total" value="{{@num_format($qty*$demand_price)}}">
    </td>
