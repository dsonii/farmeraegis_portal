...
<div class="modal-body">
    ...
    <div class="row">
      ...
      <div class="col-sm-4">
        ...
        @else
          @if(isset($sell->contact->landmark))
              {{ $sell->contact->landmark }},
          @endif

          {{ $sell->contact->city ?? '' }}

          @if(isset($sell->contact->state))
              {{ ', ' . $sell->contact->state ?? '' }}
          @endif
          <br>
          @if(isset($sell->contact->country))
              {{ $sell->contact->country ?? ''}}
          @endif
          @if(isset($sell->contact->mobile))
          <br>
              {{__('contact.mobile')}}: {{ $sell->contact->mobile }}
          @endif
          @if(isset($sell->contact->alternate_number))
          <br>
              {{__('contact.alternate_contact_number')}}: {{ $sell->contact->alternate_number ?? ''}}
          @endif
          @if(isset($sell->contact->landline))
            <br>
  ...
  <div class="modal-footer">
    @if(isset($_GET['type']) && $_GET['type'] === 'orders')
      <button type="button" class="btn btn-default btn-order-update" data-href="{{action('Restaurant\OrderController@markAsServed', [$sell->id])}}">Update</button>
      @endif
