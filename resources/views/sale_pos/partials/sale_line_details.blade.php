<form id="frm_update_pack_status">
<table class="table bg-gray">
    ...
    @foreach($sell->sell_lines as $key => $sell_line)
        <tr>
            ...
            @if(isset($_GET['shipment']) && $_GET['shipment'] === 'true')
                {{$_SERVER['QUERY_STRING']}}
            @endif
            @if(isset($_GET['type']) && $_GET['type'] === 'orders')
            <td>
                <div class="qty mt-5">
                    <input type="hidden" name="transaction_sell_line_id[]" value="{{$sell_line->id }}">
                    <input type="number" name="pack_qty[]" max="{{ (int) $sell_line->quantity }}" min="0" />
                </div>
            </td>
            @else
                @if(!empty($pos_settings['inline_service_staff']))
