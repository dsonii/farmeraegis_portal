	...
	$(document).ready( function() {
		...
		@if(!empty($shortcuts["pos"]["recent_product_quantity"]))
			...
			Mousetrap.bind('{{$shortcuts["pos"]["recent_product_quantity"]}}', function(e, combo) {
				...
				var length_now = $('table#demand_entry_table tr').length;
				...
				var last_qty_field = $('table#demand_entry_table tr').eq(shortcut_position_now - 1).contents().find('input.demand_quantity');
			...
			// var timeout = null;
			// $('table#demand_entry_table').on('focus', 'input.demand_quantity', function () {
			//     var that = this;
